 ...
 * Rendered once, by the root layout, as a direct flex child of <body> — which
 * is what makes it span the viewport rather than the centred content column.
 *
 * It reads the cluster off the URL, and the dossier guarantees the URL tells
 * the truth: a devnet program redirects to carry ?network=devnet, a mainnet one
 * strips a stale devnet param (see app/p/[id]/page.tsx). That matters because
 * this used to key purely off browsing mode, so opening a devnet program while
 * in mainnet mode showed no devnet indication anywhere on the page — the worst
 * way for the one component whose job is making the cluster unambiguous to be
 * wrong.
 */
